...
const TopNav = () => {
  ...
  const sideMenuToggleFunction = () => {
    setSideMenuToggle({
      ...
      ['toggle']: true,
      ['bgColor']:
        'bg-yellow1 animate-wiggledown inset-0 top-0 left-0 absolute z-10',
    });
    if (sideMenuToggle['toggle'] == true) {
      setSideMenuToggle({
        ...
        ['toggle']: false,
        ['bgColor']:
          'bg-white duration- opacity-70 delay-700 animate-wiggleup inset-0 top-0 left-0 absolute z-10',
      });
    }
  };
  ...
  const dalayHiddenHamburgerMenu = useEffect(() => {
    try {
      if (sideMenuToggle['toggle'] !== undefined) {
        ...
        setTimeout(() => {
          if (sideMenuToggle['toggle'] == false)
            toggleSideMenu.current.className = ' hidden';
        }, 900);
      }
    } catch (e) {}
  ...
  return (
    <>
      <div className="flex justify-between ">
        ...
        <div
          onClick={sideMenuToggleFunction}
          className="relative z-30 h-[30px] w-[30px] mt-[28px] mr-[28px]"
        >
          {sideMenuToggle['toggle'] ? (
            <Image src={'/cancelx.svg'} layout={'fill'} />
          ) : (
